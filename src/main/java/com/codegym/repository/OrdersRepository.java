...
public interface OrdersRepository extends JpaRepository<Orders, Long> {
    ...
    @Query(value = "select sum(op.quantity * f.price) from orders o inner join order_product op on op.order_id = o.id join orders_shops os on os.orders_id = o.id inner join food f on f.id = op.food_id where o.status = 'DONE' and os.shops_id = :id",nativeQuery = true)
    Double getTotalProfitByShopId(@Param("id") Long id);
